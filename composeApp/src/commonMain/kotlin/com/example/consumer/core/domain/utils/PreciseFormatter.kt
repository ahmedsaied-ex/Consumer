...
import com.example.consumer.core.domain.model.PreciseDecimal
...
fun formatFiatPrecise(
    ...
    return when {
        !showDecimal -> {
            val formatted = formatWithCommasDefault(absAmount.toDisplayString(0))
            "$sign $currencySymbol $formatted"
        }
        else -> {
            val formatted = formatWithCommasDefault(absAmount.toDisplayString(2))
            "$sign $currencySymbol $formatted"
        }
    }
}
...
fun formatCryptoPrecise(
    ...
    val withCommas = formatWithCommasDefault(formatted)
...
fun formatPriceDisplay(
    ...
    return when {
        absPrice >= PreciseDecimal.fromString("1000000") -> {
            ...
            val formatted = formatWithCommasDefault(absPrice.toDisplayString(2))
            "$sign$currencySymbol$formatted"
        }
...
fun formatDouble(value: Double): String {
    ...
    val plain = rounded.toBigDecimal().toPlainString()
    ...
    return if (decimalPart.isEmpty()) parts[0]
    else "${parts[0]}.$decimalPart"
}

fun formatWithCommasDefault(numberString: String): String {
    val isNegative = numberString.startsWith("-")
    val absString = if (isNegative) numberString.removePrefix("-") else numberString

    val parts = absString.split(".")
    val integerPart = parts[0]
    val decimalPart = if (parts.size > 1 && parts[1].isNotEmpty()) ".${parts[1]}" else ""
    ...
    val formatted =
        when {
            absAmount >= PreciseDecimal.fromString("1000") -> {
                formatWithCommasDefault(absAmount.toDisplayString(2))
            }
            else -> {
